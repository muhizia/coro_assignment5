...
SimulatorCamera::SimulatorCamera()
{
   ...
   if (debug)
      printf("SimulatorCamera constuctor\n");
...
void SimulatorCamera::initialize()
{
   ...
   if (debug)
      printf("SimulatorCamera::initializeCamera\n");
...
void SimulatorCamera::imageMessageReceived(const sensor_msgs::ImageConstPtr &msg)
{
   ...
   if (debug)
      printf("SimulatorCamera::imageMessageReceived: image %ld size is %d x %d\n", frameNumber, frame.rows, frame.cols);
}
...
void SimulatorCamera::getImage(Mat &image)
{
   ...
   if (debug)
      printf("SimulatorCamera::getImage: image %ld size is %d x %d\n", lastFrameAcquired, image.rows, image.cols);
};
...
bool computeJointAngles(double x, double y, double z, double pitch_angle_d, double roll_angle_d, double joint_angles[])
{
   ...
   if (debug)
      printf("computeJointAngles(): x %4.1f, y %4.1f, z %4.1f, pitch %4.1f, roll %4.1f\n", x, y, z, pitch_angle_d, roll_angle_d);
...
{
   if ((int)x <= MAX_X && (int)x > MIN_X && (int)y <= MAX_Y && (int)y > MIN_Y && (int)z <= MAX_Z && (int)z > MIN_Z)
      return 1;
   else
      return 0;
}
...
bool getJointPositions(float x, float y, float z, float pitch_angle_d, float roll_angle_d, int positions[])
{
   ...
   if (false && debug)
      printf("getJointPositions(): %4.1f %4.1f %4.1f %4.1f %4.1f\n", x, y, z, pitch_angle_d, roll_angle_d);
...
int gotoPose(float x, float y, float z, float pitch, float roll)
{
   ...
   if (debug)
      printf("gotoPose(): %4.1f %4.1f %4.1f %4.1f %4.1f\n", x, y, z, pitch, roll);
...
void readRobotConfigurationData(char filename[])
{
   ...
   for (i = 0; i < NUMBER_OF_KEYS; i++)
   {
      ...
      for (j = 0; j < NUMBER_OF_KEYS; j++)
      {
         if (strcmp(key, keylist[j]) == 0)
         {
            ...
            {
            case 0:
               sscanf(input_string, " %s %s", key, robotConfigurationData.com); // com
               break;
            case 1:
               sscanf(input_string, " %s %d", key, &(robotConfigurationData.baud)); // baud
               break;
            case 2:
               sscanf(input_string, " %s %d", key, &(robotConfigurationData.speed)); // speed
               break;
            case 3:
               sscanf(input_string, " %s %d %d %d %d %d %d", key, // channel
               ...
               break;
            case 4:
               sscanf(input_string, " %s %d %d %d %d %d %d", key, // home
               ...
               break;
            case 5:
               sscanf(input_string, " %s %f %f %f %f %f %f", key, // degree
            ...
            case 7:
               ...
               break;
            case 8:
               sscanf(input_string, " %s %f %f %f %f %f %f", key, // current_joint_value
   ...
   if (debug)
   {
      ...
      printf("CHANNEL:  ");
      for (k = 0; k < 6; k++)
         printf("%d ", robotConfigurationData.channel[k]);
      printf("\n");
      printf("HOME:     ");
      for (k = 0; k < 6; k++)
         printf("%d ", robotConfigurationData.home[k]);
      printf("\n");
      printf("DEGREE:   ");
      for (k = 0; k < 6; k++)
         printf("%3.1f ", robotConfigurationData.degree[k]);
      printf("\n");
      printf("EFFECTOR: ");
      ...
      printf("CURRENT:  ");
      for (k = 0; k < 6; k++)
         printf("%4.3f ", robotConfigurationData.current_joint_value[k]);
      printf("\n");
   }
}
...
int spawn_brick(std::string name, std::string color, double x, double y, double z, double phi)
{
   ...
   {
      if (debug)
         ROS_INFO("Spawned brick [%s] of color [%s] at position (%.3f %.3f %.3f %.2f %.3f %.3f)", srv.response.name.c_str(), color.c_str(), (x / 1000.0), (y / 1000.0), (z / 1000.0), 0.0, 0.0, radians(phi));
      wait(1000);
   }
   else
   {
...
int kill_brick(std::string name)
{
   ...
   {
      if (debug)
         ROS_INFO("Killed brick [%s]", name.c_str());
   }
   else
   {
      ROS_ERROR("Failed to call the service");
...
/**
 * red green blue to hue saturation and intensity
 * Taken from class codes from Prof. David Vernon.
*/
 ...
 * This function returns vector of vectors of points which is the container of all coutours found in the picture.
...
void ContourExtraction(cv::Mat src, std::vector<std::vector<cv::Point>> *contours)
{
   ...
   inRange(src_hue, cv::Scalar(0, 112, 112), cv::Scalar(180, 255, 255), src_threshold);
                    ...
                    float grasp_x, float grasp_y, float grasp_z, float grasp_theta)
{
   ...
   if (debug)
      printf("Moving to object approach pose\n");
   ...
   if (debug)
      printf("Moving to object depart pose\n");
   ...
   if (debug)
      printf("Moving to destination approach pose\n");
   ...
   if (debug)
      printf("Moving to destination pose\n");
   ...
   if (debug)
      printf("Moving to destination depart pose\n");
